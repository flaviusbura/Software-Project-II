...
public class StatistiekView {
	...
	public static int setStatistiekresultaat(ArrayList<Abonnement> list){
		...
		ArrayList<Double> omzet=new ArrayList();
		for(int i=0;i<list.size();i++){
			...
			if(found){
				...
				double newOmzet = omzet.get(foundPlace)+abonnementDAO.getTotaalprijsAbonnement(typen.get(foundPlace).getAbonnementId());
				Abonnement abo = typen.get(foundPlace);
				totalen.remove(foundPlace);
				typen.remove(foundPlace);
				omzet.remove(foundPlace);
		...
		for (int i=0;i<totalen.size();i++){
			result = result + abonnementTypeDao.getTypeVanAbonnement(typen.get(i).getAbonnementId()) + ": "+totalen.get(i) + "totaalprijs: "+ omzet.get(i)+ "/n";
			totaalomzet +=omzet.get(i);
		}
		result ="/n"+"Totaalomzet: "+totaalomzet;
